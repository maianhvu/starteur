class Educators::Admin::SearchController < Educators::Admin::BaseController
  ...
  def index
    ...
    if search_params.any?
      @organisations = Kaminari.paginate_array(Educator.where("lower(organisation) like ?", "%#{search_params[:search_text].downcase}%").pluck(:organisation).uniq).page(params[:org_page]).per(num_per_page)
